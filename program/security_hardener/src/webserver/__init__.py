...
from security_hardener.src.webserver import actions

def harden():

    pl = parse_template(ctx=context_config["webserver"])
    pc = parse_compiler(ctx=context_config["webserver"])
    ps = parse_template(ctx=context_config["webserver"], parse_file="system")
