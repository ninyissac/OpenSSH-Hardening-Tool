...
from security_hardener.src.gitserver import actions

def harden():

    pl = parse_template(ctx=context_config["gitserver"])
    pc = parse_compiler(ctx=context_config["gitserver"])
    ps = parse_template(ctx=context_config["gitserver"], parse_file="system")
